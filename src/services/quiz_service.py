...
def get_all_quizzes():
    ...
    quizzes_ref= db.collection('study_quizzes')
    quizzes = []
...
def get_quiz_by_id(id: str):
    ...
    quiz_ref = db.collection('study_quizzes').document(id)
    quiz_dict = quiz_ref.get().to_dict()

    questions_ref = db.collection("study_quizzes", quiz_ref.id, "questions")
    program_ref = quiz_dict.get('program')
    
    questions = []
    for question_doc in questions_ref.stream():
        question = question_doc.to_dict()
        choices_ref = db.collection("study_quizzes", quiz_ref.id, "questions", question_doc.id, "choices")
...
def create_quiz(program_id, file_content):
        ...
        program_doc_ref = db.collection('study_programs').document(program_id)
        new_quiz_data = { 'program' : program_doc_ref }
        new_quiz_doc_ref= db.collection('study_quizzes').document()
        ...
        for i, question in enumerate(quiz_questions):
            ...
            new_question_doc_ref = db.collection('study_quizzes', new_quiz_doc_ref.id, 'questions').document(str(i))
            new_question_doc_ref.set(new_question_data)
            
            for j, choice_json in enumerate(question_json.get('choices')) :
                new_choice_doc_ref = db.collection('study_quizzes', new_quiz_doc_ref.id, 'questions', new_question_doc_ref.id, 'choices' ).document(str(j))
...
def create_quiz_submission(create_submission: CreateSubmission):
    # create quiz data
    db = firestore.client()
    new_submission_ref = db.collection('study_quizzes', create_submission.quiz_id, 'submissions' ).document()
    submission_data = {
